 ...
 * 功能：文件添加fragment
 */
class AddVideoFragment : BaseFragment<AddFilePresenter, AddFileModel>(), AddFileContract.View {
    ...
    companion object {
         ...
         * 启动器
         */
        fun newInstance(fileType: FileType = FileType.NORMAL, maxFileNum: Int = 10, selectFileTitle: String = ""): AddVideoFragment {
            val fragment = AddVideoFragment()
            val bundle = Bundle()
            bundle.putSerializable("fileType", fileType)
            bundle.putInt("maxFileNum", maxFileNum)
            if (selectFileTitle.isNotEmpty()) bundle.putString("selectFileTitle", selectFileTitle)
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        if (arguments!!.containsKey("selectFileTitle")) {
            tv_selectFile.text = arguments!!.getString("selectFileTitle")
        }
    }
     ...
     * 编辑的回调
     */
    fun setOnEditCallBack(eidtCallBack: (FileAction) -> Unit) {
        ...
        if (requestCode == 0x03) {//文件选取
            if (data != null) {
                ...
                if (filePath == null) {
                    ...
                    val type = if (filePath.endsWith("jpg") || filePath.endsWith("png")) {
                        1
                    } else if (filePath.endsWith("avi") || filePath.endsWith("mp4") || filePath.endsWith("rmvb") || filePath.endsWith("3gp")) {
                        2
                    } else {
                        3
                    }
                    val fileBean = AddFileBean(fileName, type, filePath, filePath)
